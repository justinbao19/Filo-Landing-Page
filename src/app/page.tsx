...
export default function Home() {
  ...
  useEffect(() => {
    if (!hasPlayedLineAnimation && visibleSections.has('closer-look')) {
      // 延迟触发动画，确保页面已加载
  ...
  }, [hasPlayedLineAnimation, visibleSections])
  ...
  const observerSectionRef = useRef<null | IntersectionObserver>(null)
  ...
  useEffect(() => {
    ...
    observerSectionRef.current = new IntersectionObserver((entries) => {
    ...
    sections.forEach((section) => observerSectionRef.current?.observe(section))

    return () => {
      sections.forEach((section) => observerSectionRef.current?.unobserve(section))
    }
  }, [])

  useEffect(() => {
    if (observerSectionRef.current && visibleSections.size === 7) {
      observerSectionRef.current.disconnect()
      observerSectionRef.current = null
    }
  }, [visibleSections])
  ...
  return (
    <div className="min-h-screen bg-white  w-svw">
      ...
      >
        <div className="max-w-[1440px] mx-auto px-20">
          ...
          {/* 展示区域容器 */}
          <div
            className="relative z-10 snap-center"
            style={{
              height: '1100px',
              transform: 'translateY(-50px)',
            }}
      ...
      {/* FAQ 区域 */}
      <div
        className={`${visibleSections.has('faq') ? 'visible' : ''}`}
        data-section="faq"
        style={{
          width: '100%',
          background: '#000',
      ...
      >
        <div
          className={`scroll-animate snap-center ${visibleSections.has('faq') ? 'visible' : ''}`}
          style={{
            display: 'flex',
            flexDirection: 'column',
            alignItems: 'center',
        ...
        <div className="max-w-[1280px] mx-auto">
          ...
          >
            ...
            >
              ...
              >
                No Silicon Valley buzz, no big stages, just a small team that loves building digital
                tools to make life a little easier. When AI took off, we saw a chance to make
                something simpler, smarter, more human. So we built Filo — not to change the world,
                just to make email suck less.
              </p>
